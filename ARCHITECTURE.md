...
- **セッションの自己リセット機能(2026-07-22)**: ハングした/詰まった接続を
  オペレーターの手動SSH介入なしにブラウザ側から解消できるよう、`broker.sh`
  に第2引数(任意)を追加した。ttyd が `?arg=` を複数回受け取れる仕組み
  (`?arg=<IP>&arg=reset`、シェルを経由せずargvへ直接渡される)を利用し、
  `reset` という**リテラル文字列と完全一致した場合のみ**受理する(それ以外は
  IPの不正値と同様に即 `die`)。既存の `SESS="thm_$(...)"` 導出をそのまま
  再利用して `tmux kill-session` し、その後は**既存の**
  has-session/new-session 分岐へそのまま処理を渡す(分岐自体は複製しない)。
  上記の `ssh -e none` とは完全に独立した経路であり、sshのエスケープ
  シーケンスを一切再開しない — tmux層のみで完結する、明示的に検証済みの操作。
  新たな権限境界は生じない: 共有クレデンシャルの保持者は元々
  `attach-session -d` で他クライアントを強制的に追い出せる(下記の脅威モデル
  参照)ため、「kill してから作り直す」のはその強い操作の部分集合に過ぎない
  (作業内容/スクロールバックが失われる点のみが新規の影響)。`caddy/form.html`
  に既定オフのチェックボックスを追加し、破壊的操作である旨を明記した。加えて
  `thmctl reset <ip>` をオペレーター側CLIにも追加したが、こちらは
  kill のみで再接続はしない(ブラウザ側と挙動を分けている)。
  **実装時のインシデント**: `thmctl reset` の初版は
  `su ephemeral -s /bin/sh -c '...-t "$1"' -- "$sess"` という形で位置引数を
  渡そうとしたが、BusyBox `su` 環境ではこの `$1` が空文字列に展開され、結果
  `tmux kill-session -t ""` 相当が実行されて **tmuxサーバー全体(稼働中の
  実セッションを含む全て)が落ちる**実害が出た(2026-07-22、実機で発生・
  即座に検知)。原因を`su -c`スクリプト内での位置引数展開の不備と特定し、
  対象IPを事前に `[0-9.]` のみに制限した上で `$sess` を直接ダブルクォート
  文字列へ埋め込む方式(このファイルの他の関数と同じパターン)に修正、
  2つの独立したテストセッションを用いて「片方だけkillされ、もう片方と
  実セッションは無傷」であることを確認してから再デプロイした。VPN/ttyd
  自体は無停止だったため実害はブラウザ側スクロールバックの喪失のみ
  (再接続で即復旧)だが、`thmctl`の`su`呼び出しパターンを変更する際は
  同様の位置引数の罠に注意すること。
